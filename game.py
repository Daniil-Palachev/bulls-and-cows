...
def game_bot(token):
    ...
    def start(update, context):
        global word
        word = random.choice(list(open('Listofwords.txt')))
        print(word)
        context.bot.send_message(update.effective_chat.id, "Привет! Правила игры просты. Я загадал слово из пяти неповторяющихся букв, твоя задача - угадать слово, называя слова из того же количества букв. Если буква из вашего слова есть в загаданном - это корова. Если еще и позиция совпадает - это бык. Соберешь 5 быков и ты - победитель!")
        ...
        if len(word_new) == 5:
            for i,j in zip(word, word_new):
                if j in word:
                    if i == j:
                        bull += 1
                        context.bot.send_message(update.effective_chat.id, f'Вы нашли {bull} быков')
                    else:
                        cow += 1
                        context.bot.send_message(update.effective_chat.id, f'Вы нашли {cow} коров')
            if bull == 5:
                context.bot.send_message(update.effective_chat.id, f'Поздравляю! ты меня рассекретил {word}!')
                return ConversationHandler.END
            else:
                context.bot.send_message(update.effective_chat.id, f'Вы не угадали, попробуй еще!')
                return MATCH
        else:
